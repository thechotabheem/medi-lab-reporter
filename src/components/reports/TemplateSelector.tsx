...
import { reportTemplates, activeReportTypes, registerCustomTemplates, getAllActiveTemplates } from '@/lib/report-templates';
...
import { 
  Droplets, Heart, Activity, TestTube, Droplet, ShieldCheck, 
  Bug, Syringe, FlaskConical, CircleDot
} from 'lucide-react';
...
export const TemplateSelector = ({ 
  ...
  // Register custom templates into the unified registry whenever they load
  useEffect(() => {
  ...
  // Build unified list: built-in + custom, sorted alphabetically
  const allTemplates = useMemo(() => {
    return getAllActiveTemplates();
  }, [fullyCustomTemplates]); // re-compute when custom templates load

  // Filter unified list
  const filteredTemplates = useMemo(() => {
    if (!debouncedSearch.trim()) return allTemplates;
  ...
  const getFieldCount = (code: string): number => {
    // Check built-in
    const builtIn = reportTemplates[code as ReportType];
    if (builtIn) {
      return builtIn.categories.reduce((sum, c) => sum + c.fields.length, 0);
    }
    // Check custom
    const custom = fullyCustomTemplates?.find(t => t.code === code);
    if (custom) {
      return custom.categories.reduce((sum, c) => sum + c.fields.length, 0);
    }
    return 0;
  };
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2 p-3 bg-primary/10 rounded-lg border border-primary/20">
          ...
          <div className="flex-1 flex flex-wrap gap-1">
            {selectedTypes.map(type => {
              const entry = allTemplates.find(t => t.code === type);
              return (
      ...
      <div className="space-y-2">
        ...
        {/* Quick Custom Tests already added to this report */}
        ...
        {filteredTemplates.map((entry) => {
          ...
          const fieldCount = getFieldCount(entry.code);
          
          return (
            <div
              key={entry.code}
              onClick={() => handleToggle(entry.code)}
            ...
            >
              ...
              <div className={`p-2 rounded-lg ${selected ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                {templateIcons[entry.code] || <Beaker className="h-5 w-5" />}
              </div>
        ...
        {filteredTemplates.length === 0 && customTests.length === 0 && (
