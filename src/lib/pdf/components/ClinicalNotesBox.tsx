...
export const ClinicalNotesBox: React.FC<ClinicalNotesBoxProps> = ({ notes, fontSizeMultiplier = 1 }) => (
  <View style={[tw('mt-3 p-2'), { borderWidth: 0.5, borderColor: '#d2d2d2', borderRadius: 2 }]}>
    <Text style={[tw('font-bold'), { fontSize: 10 * fontSizeMultiplier, color: '#282828', fontFamily: FONTS.heading }]}>
      Clinical Notes:-
    </Text>
    <Text style={{ fontSize: 9 * fontSizeMultiplier, color: '#282828', marginTop: 3, fontFamily: FONTS.body }}>
      {notes}
    </Text>
  </View>
);
